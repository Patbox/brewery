...
public record AlcoholValueEffect(boolean replace, List<Value> entries, Map<Item, Double> itemReduction, Map<Item, Identifier> ingredientMixModels) {
    public static Codec<AlcoholValueEffect> CODEC = RecordCodecBuilder.create(instance ->
            instance.group(
                    ...
                    Codec.unboundedMap(BuiltInRegistries.ITEM.byNameCodec(), Codec.DOUBLE).optionalFieldOf("alcohol_reduction_items", Map.of()).forGetter(AlcoholValueEffect::itemReduction),
                    Codec.unboundedMap(BuiltInRegistries.ITEM.byNameCodec(), Identifier.CODEC).optionalFieldOf("container_to_ingredient_mix_models", Map.of()).forGetter(AlcoholValueEffect::ingredientMixModels)
